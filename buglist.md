...
## [x] B15. 세로 모니터에서 ← 이동 시 바로 왼쪽 모니터가 아니라 내장 디스플레이로 떨어진다

- **재현 조건**: 4-모니터 배치 — 와이드(G5) · 와이드(G4) 가 좌→우로,
  그 오른쪽에 세로(portrait) 모니터, 내장 디스플레이는 세로 모니터의
  좌하단 대각선 위치 (primary). 가운데 와이드 모니터의 창에서 `→` 로
  세로 모니터 창으로 이동한 뒤 `←` 를 누른다.
- **관찰된 동작**: 커서가 출발했던 가운데 모니터 창이 아니라 내장
  디스플레이의 창으로 이동한다 (왕복 비대칭).
- **기대 동작**: `→` 다음 `←` 는 원래 창으로 돌아와야 한다. 내장
  디스플레이 창은 `↓` 로 도달하는 것이 자연스럽다.
- **원인 (실좌표 계산으로 확정)**: `FocusWindowGeometry.nextWindow` 의
  순위가 순수 유클리드 midpoint 거리였다. 세로 모니터 창 중심
  (2160,-884) 기준으로 가운데 창 (160,-720) 은 거리 2006.7, 내장 창
  (720,450) 은 dx=-1440/dy=+1334 의 ~43° 대각선인데도 거리 1962.9 로
  근소하게 더 가까워 이긴다. quadrant gate 는 두 후보 모두 통과시키므로
  (B7 의 0.5 완화 이전 엄격 gate 로도 1440>1334 통과) gate 가 아니라
  ranking 의 문제.
- **해결 방식**: ranking 거리에 직교축 2× 가중 — `←/→` 는
  `√(dx²+(2dy)²)`, `↑/↓` 는 `√((2dx)²+dy²)`. 요청 방향에 정렬된 창이
  비슷한 거리의 대각선 창을 이긴다. gate 는 raw dx/dy 그대로라 B7 류
  대각선-유일 후보의 도달성은 유지 (B7 예시는 두 후보의 dy 가 같아
  순위 불변).
- **관련 파일**:
  - `Rectangle/MultiWindow/FocusWindowGeometry.swift`
  - `RectangleTests/FocusWindowGeometryTests.swift`
